...
- **저장소에는 `.sog` 만 올렸습니다.** 나머지 세 형식은 같은 `.ply` 에서 만들어 위 값을
  재고 뷰어로 확인한 뒤 뺐습니다. 다시 만들려면 아래 "어떻게 다시 만드나"를 보십시오.
...
고르는 선택지가 없습니다. `.spz` 를 만들어 뷰어 창에 끌어다 놓으면 위 문구가 빨간 상자로 뜹니다.

`.compressed.ply` 는 정상으로 열립니다. 뷰어는 **파일을 창에 끌어다 놓으면 그 파일로
다시 그리므로**, 세 형식을 직접 만들어 눈으로 견주어 볼 수 있습니다.

## 폰에서 첫 화면까지

주소를 누른 순간부터 추모관 벽이 화면에 보일 때까지 잰 값입니다.
...
처음과 두 번째가 갈리는 까닭은 **브라우저가 받아 둔 것을 다시 쓰기 때문**입니다.
처음에는 13.89 MiB짜리 `scene.sog` 를 통째로 내려받아야 하지만, 두 번째부터는
그 파일이 폰에 남아 있어 받는 시간이 거의 0이 되고 알갱이를 푸는 시간만 듭니다.

참고로 유선에서 같은 파일을 받는 데는 0.29초, 로컬 서버에서는 0.05초였습니다.
즉 **처음 5~7초 중 대부분이 내려받는 시간**이고, 폰끼리의 2초 차이는 그날의
회선 속도 차이로 보입니다. 알갱이 337만 개를 푸는 시간은 어느 폰에서나 비슷하게 듭니다.
...
3. **줄이기** (GPU 불필요). `-w` 를 빼면 파일이 이미 있을 때 덮어쓰지 않고 멈춥니다.

```bash
npx -y @playcanvas/splat-transform@latest -w scene.ply scene.sog
npx -y @playcanvas/splat-transform@latest -w scene.ply scene.compressed.ply
npx -y @playcanvas/splat-transform@latest -w scene.ply scene.spz
```
